...
export default function FootballMapsHeroSection() {
  const { t } = useTranslation();
  const paragraphs = t("modules.footballMaps.paragraphs", { returnObjects: true });
  const title = t("modules.footballMaps.title");
  const cta = t("modules.footballMaps.cta");
  const badge = t("hero.map.badge", { defaultValue: "FootballMaps" });

  return (
    <section className="w-full bg-transparent text-white">
      <div className="relative isolate overflow-hidden">
        <div
          className="absolute inset-0 bg-gradient-to-r from-[#0b0c0f] via-[#0f1116] to-[#0b0c0f] opacity-70"
          aria-hidden="true"
        />

        {/* Soft glow on the right */}
        <div
          className="pointer-events-none absolute right-[-14%] top-1/2 h-80 w-80 -translate-y-1/2 rounded-full bg-[radial-gradient(circle_at_center,_rgba(163,255,0,0.18),_transparent_60%)] blur-3xl opacity-70"
          aria-hidden="true"
        />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 md:px-8 pt-24 pb-16 sm:pb-20 md:pb-24">
          <div className="max-w-3xl mx-auto md:mx-0 px-0 sm:px-4 md:px-0 text-center md:text-left space-y-6">
            ...
            {/* Description (your text, split into clear paragraphs) */}
